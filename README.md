...
- BUFR 形式の電文の読み込み
- 非同期 API
  - 電文は大きいものでも数 MB で、読み込みは[性能](#性能)のとおり数十ミリ秒以内で終わります。非同期の `XmlReader` に対応するには、生成したコードを含む読み込みのコードを非同期用にもう 1 組用意する必要があります。そのため同期 API だけを提供しています。
...
次の表は、同梱した公式サンプル電文のうち 3 通を `Report.Parse(string)` で読み込んだときの平均時間と割り当て量です。割り当て量は 1 回の読み込みで確保するマネージドメモリの総量で、同時に使用するメモリの最大値ではありません。サイズと割り当て量は 1 KB = 1,024 バイトで換算しています。

| 電文 | サイズ | 平均時間（Apple M4） | 平均時間（Intel Core i5-12600K） | 割り当て量 |
|---|---:|---:|---:|---:|
| 緊急地震速報（地震動予報）`77_01_27_240613_VXSE45.xml` | 22 KB | 0.053 ms | 0.060 ms | 69 KB |
| 震源・震度に関する情報 `32-39_11_05_240613_VXSE53.xml` | 388 KB | 1.0 ms | 1.1 ms | 932 KB |
| 気象警報・注意報（集約通報）`15_18_01_250630_VPWS50.xml` | 4.4 MB | 15 ms | 24 ms | 12.8 MB |

大きな電文では、読み込みの途中で GC（ガベージコレクション）が何度も実行されます。GC は実行のたびに作成中の結果（多数の小さなオブジェクト）を調べて古い世代へ移すので、その分の時間がかかります。そのため、アプリケーションの GC の設定によって読み込みの時間が変わることがあります。

計測環境：.NET 10.0.12、BenchmarkDotNet 0.15.8、ワークステーション GC（バックグラウンド GC 有効）。OS は Apple M4 が macOS 27.0（Arm64）、Intel Core i5-12600K が Windows 11 10.0.29671（x64）です。

次のコマンドで計測できます。

```bash
dotnet run -c Release --project bench/JmaXml.Benchmarks -- --filter '*'
```
